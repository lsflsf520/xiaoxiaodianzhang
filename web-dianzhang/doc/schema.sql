...
create table shop(
  ...
  `county_id` int(10) unsigned comment '所属城市下属行政区域id。',
  `detail_addr` varchar(200) comment '详细街道门牌号地址',
  `lng` decimal(10,7) comment '小店位置所在经度',
  `lat` decimal(10,7) comment '小店位置所在纬度',
  `wx_uid` int(10) unsigned NOT NULL DEFAULT 0 COMMENT '小店所属微信用户id',
  `snapshot` varchar(2000) comment '快照。在用户申请修改',
  `status` enum('CLOSED', 'REJECT', 'CHECKING', 'PASSED') COMMENT '数据状态。已关闭、已拒绝、待审核、已审核通过',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间',
  `last_uptime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最近一次更新时间',
  PRIMARY KEY (`id`)
...
create table coupon_tmpl(
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '小店ID，自增长',
  `coupon_type` enum('YouHui', 'LiJian') comment '券类型。优惠券、立减券',
  `tmpl_file_name` varchar(32) not null comment '券对应的样式模板文件名',
  `discount_type` enum('Amount', 'Discount') comment '折扣类型。面额、折扣',
  `discount` decimal(8, 2) comment '折扣值。如果discount_type为Amount,此处为面额；如果discount_type为Discount，则此处为折扣值（比如7折，此处存储为70.00）'
  `min_amount` decimal(11, 2) default 100 comment '使用该券，要求的最小消费金额。默认为100.00元',
...
create table coupon_order(
  ...
  `pay_fee` decimal(11, 2) default 0.00 comment '本次购买需要支付的总金额。默认为0.00',
  `pay_method` set('BALANCE','WXPAY','ALIPAY','UNIONPAY') not null comment '支付方式。余额支付、微信支付、支付宝支付、银联支付。可以组合支付，但只能有余额+另外一种第三方支付方式',
  `balance_pay_fee` decimal(11, 2) default 0.00 comment '本次购买使用余额支付的金额。默认为0.00',
  `third_pay_fee` decimal(11, 2) default 0.00 comment '本次购买使用第三方支付的金额。默认为0.00',
...
create table wallet(
  ...
  `balance` decimal(11, 2) default 0.00 comment '用户余额。默认为0.00',
  `total_charge_amount` decimal(11, 2) default 0.00 comment '用户总充值金额。默认为0.00',
  `status` enum('NORMAL', 'FREEZED', 'DELETED') COMMENT '数据状态。正常、已冻结、已删除',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间',
  `last_uptime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最近一次更新时间',
  PRIMARY KEY (`id`)
...
create table cash_log(
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID，自增长',
  `wx_uid` int(10) unsigned NOT NULL COMMENT 'wx_user表的id',
  `cash_amount` decimal(11, 2) not null comment '用户申请提现的金额。',
  `pre_amount` decimal(11, 2) not null default 0.00 comment '申请提现之前账户的可用余额',
  `after_amount` decimal(11, 2) not null default 0.00 comment '申请提现之后账户的可用余额',
  `last_cash_left_amount` decimal(11, 2) not null default 0.00 comment '上次申请提现之后账户的可用余额',
...
create table charge_log (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID，自增长',
  `wx_uid` int(10) unsigned NOT NULL COMMENT 'wx_user表的id',
  `charge_amount` decimal(11, 2) not null comment '用户充值金额。',
  `pre_amount` decimal(11, 2) not null default 0.00 comment '本次充值之前账户的可用余额',
...
create table suggestion (
  ...
  `status` enum('HANDLING', 'HANDLED') not null default 'HANDLING' comment '对用户建议的处理状态。待处理、已处理',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间',
  `last_uptime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最近一次更新时间',
  PRIMARY KEY (`id`)
...
create table user_coupon(
  ...
  `src_code` varchar(64) default 'SHOP_SCAN' comment '优惠券来源的CODE。默认为 SHOP_SCAN(店铺扫码)',
  `coupon_type` enum('YouHui', 'LiJian') comment '券类型。优惠券、立减券',
  `tmpl_file_name` varchar(32) not null comment '券对应的样式模板文件名',
  `discount_type` enum('Amount', 'Discount') comment '折扣类型。面额、折扣',
  `discount` decimal(8, 2) comment '折扣值。如果discount_type为Amount,此处为面额；如果discount_type为Discount，则此处为折扣值（比如7折，此处存储为70.00）'
  ...
  `can_use_fee` decimal(11, 2) comment '本次消费符合本券使用范围的商品金额。',
  `left_pay_fee` decimal(11, 2) comment '本次消费剩余需要支付的金额。ord_fee = left_pay_fee + 折扣金额或面额',
  `pay_way` enum('ONLINE_PAY','OFFLINE_PAY') not null comment '支付方法。在线支付，线下支付。',
  `status` enum('DELETED', 'NORMAL', 'USED') COMMENT '数据状态。已删除、正常、已使用',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据创建时间',
  `last_uptime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '最近一次更新时间',
  PRIMARY KEY (`id`)
...
create table user_pay_order (
  `user_coupon_id` int(10) unsigned NOT NULL COMMENT 'user_coupon表的ID，自增长',
  `wx_uid` int(10) unsigned NOT NULL COMMENT 'wx_user表的id',
  `pay_fee` decimal(11, 2) not null comment '用户支付的订单金额，也就是user_coupon表的left_pay_fee字段的值',
  `balance_pay_fee` decimal(11, 2) default 0.00 comment '本次购买使用余额支付的金额。默认为0.00',
  `third_pay_fee` decimal(11, 2) default 0.00 comment '本次购买使用第三方支付的金额。默认为0.00',
  `pay_method` set('BALANCE','WXPAY','ALIPAY','UNIONPAY') not null comment '支付方式。余额支付、微信支付、支付宝支付、银联支付。可以组合支付，但只能有余额+另外一种第三方支付方式',
